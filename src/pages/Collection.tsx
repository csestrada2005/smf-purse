              ...
              <motion.div className="group flex flex-col" initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 1.2, ease: [0.22, 1, 0.36, 1], delay: 0.1 }} viewport={{ once: false }}>
                <Link to="/product/Drop1/W" className="flex flex-col h-full">
                  <div className="flex-1 flex items-center justify-center overflow-hidden mb-4" style={{ maxHeight: 'calc(100vh - 300px)' }}>
                    <motion.img src={indianHeritage} alt="The Indian Heritage - Woman with purse silhouette" className="max-h-full w-auto object-contain" style={{ maxHeight: '55vh' }} whileHover={{ scale: 1.03 }} transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1] }} />
                  </div>
                  <div className="text-center py-4">
                    <h3 className="text-foreground text-sm uppercase tracking-[0.2em] mb-2 font-semibold">The Indian Heritage</h3>
                    <p className="text-accent text-xs uppercase tracking-widest hover:text-foreground transition-colors duration-500">Explore</p>
                  </div>
                </Link>
              </motion.div>

              <motion.div className="group flex flex-col" initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 1.2, ease: [0.22, 1, 0.36, 1], delay: 0.3 }} viewport={{ once: false }}>
                <Link to="/product/Drop1/B" className="flex flex-col h-full">
                  <div className="flex-1 flex items-center justify-center overflow-hidden mb-4" style={{ maxHeight: 'calc(100vh - 300px)' }}>
                    <motion.img src={theClass} alt="The Class - Woman with purse and bird silhouette" className="max-h-full w-auto object-contain" style={{ maxHeight: '55vh' }} whileHover={{ scale: 1.03 }} transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1] }} />
                  </div>
                  <div className="text-center py-4">
